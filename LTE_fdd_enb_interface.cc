...
{
    stop_ports();
    boost::interprocess::message_queue::remove("DL");
    boost::interprocess::message_queue::remove("UL");

    cerr<<"LTE_fdd_enb_interface Destruct..."<<endl;
...
void LTE_fdd_enb_interface::handle_start(void)
{
    ...
    if(!started)
    {
        ...
        // Initialize message queues for debug usage
        boost::interprocess::message_queue::remove("DL");
        boost::interprocess::message_queue::remove("UL");
...
void LTE_fdd_enb_interface::handle_stop(void)
{
    ...
    if(started)
    {
        ...
        // Stop all layers
        err = radio->stop();
        if(1/*LTE_FDD_ENB_ERROR_NONE == err*/)
        {
            phy->stop();
            //mac->stop();
            //rlc->stop();
